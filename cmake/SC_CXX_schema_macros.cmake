...
macro(SCHEMA_EXES)
  ...
  if(BUILD_STATIC_LIBS)
    ...
    RELATIVE_PATH_TO_TOPLEVEL(${CMAKE_CURRENT_SOURCE_DIR} RELATIVE_PATH_COMPONENT)
    SC_ADDEXEC(p21read_${PROJECT_NAME} SOURCES "${RELATIVE_PATH_COMPONENT}/src/test/p21read/p21read.cc" LINK_LIBRARIES ${PROJECT_NAME}-static ${_libstatics} TESTABLE)
    target_compile_definitions(p21read_${PROJECT_NAME} PRIVATE SC_STATIC)
    if(NOT WIN32)
      SC_ADDEXEC(lazy_${PROJECT_NAME} SOURCES "${RELATIVE_PATH_COMPONENT}/src/cllazyfile/lazy_test.cc" LINK_LIBRARIES ${PROJECT_NAME}-static steplazyfile-static ${_libstatics} TESTABLE)
      target_compile_definitions(lazy_${PROJECT_NAME} PRIVATE SC_STATIC)
    endif(NOT WIN32)
    ...
    foreach(src ${SC_SDAI_ADDITIONAL_EXES_SRCS})
      get_filename_component(name ${src} NAME_WE)
      get_filename_component(path ${src} ABSOLUTE)
      SC_ADDEXEC(${name}_${PROJECT_NAME} SOURCES ${src} LINK_LIBRARIES ${PROJECT_NAME} ${_libstatics} TESTABLE)
      target_compile_definitions(${name}_${PROJECT_NAME} PRIVATE SC_STATIC)
      #set_target_properties(${name}_${PROJECT_NAME} PROPERTIES COMPILE_FLAGS "${${PROJECT_NAME}_COMPILE_FLAGS} -I${path}")
    endforeach(src ${SC_SDAI_ADDITIONAL_EXES_SRCS})
  endif()
